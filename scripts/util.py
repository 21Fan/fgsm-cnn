...
def text2vec(text, captcha_len=CAPTCHA_LEN, captcha_list=CAPTCHA_LIST):
    ...
    vector = np.zeros(captcha_len * LEN)      # 生成一个一维向量 验证码长度*字符列表长度
    for i in range(text_len):
        vector[captcha_list.index(text[i])%LEN+i*LEN] = 1     # 找到字符对应在字符列表中的下标值+字符列表长度*i 的 一维向量 赋值为 1
    return vector
...
def get_model(dataset='captcha'):
    ...
    if dataset == 'student':
        layers = [
            Flatten(input_shape=(CAPTCHA_HEIGHT, CAPTCHA_WIDTH, CAPTCHA_CHANNEL)),
            Dense(1024),
            Activation('relu'),
            # (8,20,64)->(1024,1)
            Dense(CAPTCHA_LEN * LEN),
            # (1024,1)->(248,1)
            Activation('sigmoid')
        ]
    elif dataset == 'captcha':
        # captcha model
        dropout=0.5
        layers = [
